########################################
if you have an mcu protocol error you need to follow the instructions here to fix it: https://github.com/billkenney/revert_qidi_software
########################################

# qidi_3series_recovery
this is a manual install script for qidi max3/plus3/smart3 firmware 4.3.15/4.2.15/4.1.13 for people with the error message that "the system starts abnormally!", a blank screen, you're unable to update the firmware for some reason, or you want an easier way to update the firmware and printer.cfg. all of the files in this repository are from qidi's github repository. the only change i made was to comment out the line "systemctl disable gpio-monitor.service" in the DEBIAN/postinst script of the mksclient, because that service did not exist on my printer and it caused the firmware update to fail
