...
if [[ "$kfg" =~ [yY] ]] ; then
  ...
  tar -xzf klipper_config.tgz ; rm klipper_config.tgz
   wget --no-check-certificate https://raw.githubusercontent.com/billkenney/qidi_3series_recovery/main/printer-max3_probe.cfg
   mv printer-max3_probe.cfg ~/klipper_config/printer.cfg
